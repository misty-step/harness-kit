...
Cheap, authed, model-native. An agent that produces a plan, artifact, doc, or
design can render a legible, on-brand image — diagrams included — in ~4s for
~$0.03. Use it; do not wrap it in a maintained script (the model writes the
call: see `model-native-product-primitives.md`).
...
- **`/shape` & "Think in HTML for plans"** — labeled architecture / sequence /
  state diagrams and system maps (NB2 renders legible in-image text).
...
- **`/groom` & `/vision`** — a one-glance system map for a `VISION.md` or moonshot.
...
- **Decoration is clutter.** If the image carries nothing the prose/table can't,
  skip it.
