...
class DocImage:
    ...
    source: str
    alt: str
    text_equivalent_anchor: str
    text_equivalent_heading: str
    text_equivalent_items: tuple[str, ...]
    source_policy: str
    review_note: str
    ...
    @property
    def output(self) -> str:
        return f"{SITE_IMAGE_ROOT}/{Path(self.source).name}"
...
def load_copy() -> dict[str, Any]:
    ...
    validate_image_declarations(data)
    return data
...
def validate_image_declarations(data: dict[str, Any]) -> None:
    images = data.get("images", [])
    if not isinstance(images, list):
        raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images must be list")
    workflow_pages = {
    ...
    for index, image in enumerate(images):
        if not isinstance(image, dict):
            raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] must be object")
        for key in [
            "page",
            "source",
            "alt",
            ...
            if key not in image:
                raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] missing {key}")
        page = str(image["page"])
        source = str(image["source"])
        if page not in workflow_pages:
            raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] targets unknown workflow page {page}")
        if not source.startswith("docs/copy/images/") or Path(source).is_absolute() or ".." in Path(source).parts:
            raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] source must stay under docs/copy/images/")
        source_path = ROOT / source
        if not source_path.exists():
            raise SystemExit(f"{source}: declared image source is missing")
        if source_path.suffix.lower() not in {".svg", ".png", ".jpg", ".jpeg", ".webp"}:
            raise SystemExit(f"{source}: unsupported image extension")
        source_size = source_path.stat().st_size
        try:
            max_bytes = int(image.get("max_bytes", 250 * 1024))
        except (TypeError, ValueError) as exc:
            raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] max_bytes must be an integer") from exc
        if max_bytes <= 0:
            raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] max_bytes must be positive")
        if source_size > max_bytes:
            raise SystemExit(f"{source}: {source_size} bytes exceeds max_bytes {max_bytes}")
        anchor = str(image["text_equivalent_anchor"])
        if not re.fullmatch(r"[a-z][a-z0-9-]*", anchor):
            raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] text_equivalent_anchor must be a stable HTML id")
        items = image["text_equivalent_items"]
        if not isinstance(items, list) or not items or any(not str(item).strip() for item in items):
            raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] text_equivalent_items must be non-empty strings")
        for text_key in ["alt", "text_equivalent_heading", "source_policy", "review_note", "regeneration_ref"]:
            if not str(image[text_key]).strip():
                raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] {text_key} must be non-empty")
        ...
        if source in seen_sources or output in seen_outputs or (page, anchor) in seen_anchors:
            raise SystemExit(f"{COPY_SOURCE.relative_to(ROOT)}: images[{index}] duplicates an image source, output, or anchor")
...
def doc_images() -> list[DocImage]:
    images: list[DocImage] = []
    for item in COPY.get("images", []):
        image = dict(item)
        images.append(
            DocImage(
                page=str(image["page"]),
                ...
                text_equivalent_items=tuple(str(value) for value in image["text_equivalent_items"]),
...
def image_manifest(image: DocImage) -> dict[str, Any]:
    ...
    return {
        ...
        "output": image.output,
        "alt": image.alt,
        "text_equivalent_anchor": image.text_equivalent_anchor,
        "text_equivalent_heading": image.text_equivalent_heading,
        "text_equivalent_items": list(image.text_equivalent_items),
        "source_policy": image.source_policy,
        "review_note": image.review_note,
        "regeneration_ref": image.regeneration_ref,
...
def render_workflow_page(workflow: dict[str, str]) -> str:
    ...
    figures = "".join(render_workflow_image(image, current) for image in doc_images() if image.page == current)
...
def render_workflow_image(image: DocImage, current: str) -> str:
    items = "".join(f"<li>{esc(item)}</li>" for item in image.text_equivalent_items)
    return f"""
    <section class="workflow-figure-section" aria-labelledby="{esc(image.text_equivalent_anchor)}">
      <figure class="workflow-figure">
        <img src="{esc(rel(current, image.output))}" alt="{esc(image.alt)}">
        <figcaption>{esc(image.review_note)}</figcaption>
      </figure>
      <div class="text-equivalent" id="{esc(image.text_equivalent_anchor)}">
        <h2>{esc(image.text_equivalent_heading)}</h2>
        <ul>
          {items}
        </ul>
        <p class="source">Image source: {source_link(image.source, current)}. {esc(image.source_policy)} Regeneration: {esc(image.regeneration_ref)}</p>
      </div>
    </section>
"""
...
def render_llms(primitives: list[Primitive], gates: list[str], images: list[DocImage]) -> str:
    lines = [
    ...
    if images:
        lines.extend(["", "Docs figures:"])
        for image in images:
            lines.append(
                f"- {image.page} | {image.source} -> docs/site/{image.output} | text: #{image.text_equivalent_anchor} | {image.alt}"
            )
...
def render_manifest(
    ...
    payload = {
        ...
        "workflows": workflows(),
        "images": [image_manifest(image) for image in images],
        "gates": gates,
        "backlog": backlog,
    }
...
def build(output: Path) -> None:
    ...
    for image in images:
        (output / image.output).parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / image.source, output / image.output)
    write_file(output, "manifest.json", render_manifest(primitives, gates, backlog, images))
    write_file(output, "llms.txt", render_llms(primitives, gates, images))
